...
class Upload {
    ...
    public function upload($files = '') {
        ...
        foreach ($files as $key => $file) {
            ...
            /* 保存文件 并记录保存成功的文件 */
            if ($this->uploader->save($file, $this->replace, $this->watermark)) {
    ...
    private function setDriver($driver = null, $config = null) {
        ...
        $class = ucfirst(strtolower($this->driver));
    ...
    private function check($file) {
        ...
        /* 检查文件长宽 */
        if (!$this->checkDetailLength($file['tmp_name'])) {
            $this->error = '上传文件尺寸不符！';
            return false;
        }
     ...
     * add by 律香川
     */
    private function checkDetailLength($tmp) {
        if ($this->width && $this->height) {
            list($width, $height, $type, $attr) = getimagesize($tmp);
            if ($width != $this->width || $height != $this->height) {
                $this->error = '请按规定尺寸上传文件！';
                return false;
            }
        }
        return TRUE;
